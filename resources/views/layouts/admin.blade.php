...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'E-Khidmat') }} - Admin Cabang</title>
        ...
        <div class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
            ...
            <header class="h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 glass-panel z-10 shadow-sm sticky top-0">
                ...
                <div class="flex items-center space-x-4 ml-auto">
                    ...
                    @php
                        // Hitung notifikasi cerdas
                        $pendingPengurusCount = \App\Models\User::where('role', 'pengurus_pac')->where('status', 'pending')->count();
                        $unreadSuratCount = \App\Models\Surat::where('penerima', 'Admin')->where('status', 'terkirim')->count();
                        $totalNotif = $pendingPengurusCount + $unreadSuratCount;
                    @endphp
                    ...
                    <div class="relative" x-data="{ notifOpen: false }">
                        ...
                        <div x-show="notifOpen" x-transition x-cloak class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-50">
                            <div class="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                                ...
                                <span class="text-xs bg-brand-100 text-brand-700 font-bold px-2 py-0.5 rounded-full">{{ $totalNotif }} Baru</span>
                            </div>
                            <div class="max-h-80 overflow-y-auto">
                                @if($pendingPengurusCount > 0)
                                <a href="{{ route('admin.pengurus.index') }}" class="block p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors">
                                    <div class="flex items-start">
                                        <div class="flex-shrink-0 bg-yellow-100 text-yellow-600 rounded-full p-2 mr-3">👤</div>
                                        <div>
                                            <p class="text-sm font-semibold text-slate-800">Menunggu Persetujuan</p>
                                            <p class="text-xs text-slate-500 mt-1">Ada {{ $pendingPengurusCount }} akun Pengurus PAC baru yang perlu Anda tinjau.</p>
                                        </div>
                                    </div>
                                </a>
                                @endif
                                ...
                                <a href="{{ route('admin.surat.masuk') }}" class="block p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors">
                                    <div class="flex items-start">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-500 mt-1">Anda memiliki {{ $unreadSuratCount }} surat masuk dari jajaran PAC.</p>
                                        </div>
                                    </div>
                                </a>
                                @endif
                                ...
                                <div class="p-6 text-center text-slate-500 text-sm">
                                    ...
                                    Tidak ada notifikasi baru.<br>Semua tugas sudah selesai!
                                </div>
                                @endif
                            </div>
                        </div>
