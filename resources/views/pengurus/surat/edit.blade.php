        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('pengurus.surat.update', $surat->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <x-text-input id="nomor_surat" name="nomor_surat" type="text" class="mt-1 block w-full" required placeholder="Contoh: 001/PAC/V/2026" value="{{ old('nomor_surat', $surat->nomor_surat) }}" />
                                <x-input-error class="mt-2" :messages="$errors->get('nomor_surat')" />
                            </div>

                            <div>
                                <x-input-label for="perihal" :value="__('Perihal Surat')" />
                                <x-text-input id="perihal" name="perihal" type="text" class="mt-1 block w-full" required placeholder="Contoh: Permohonan SK Ranting" value="{{ old('perihal', $surat->perihal) }}" />
                            ...
                            <div>
                                ...
                                <select id="penerima" name="penerima" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                    ...
                                    <option value="Admin" {{ old('penerima', $surat->penerima) == 'Admin' ? 'selected' : '' }}>Admin Cabang</option>
                                    @foreach($pacs as $pac)
