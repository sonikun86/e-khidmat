            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h3 class="text-lg font-bold">Pengajuan & Surat Keluar</h3>
                            <p class="text-sm text-gray-500">Surat yang Anda buat untuk dikirimkan.</p>
                        </div>
                        <a href="{{ route('pengurus.surat.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition">
                            + Buat Surat Baru
                        </a>
                    </div>
                                ...
                                @forelse($surats as $surat)
                                    <tr class="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-center space-y-1">
                                            ...
                                            @if($surat->status == 'draft')
                                                <a href="{{ route('pengurus.surat.edit', $surat->id) }}" class="block w-full px-3 py-1 bg-yellow-500 text-white text-xs rounded hover:bg-yellow-600 text-center">Edit Draft</a>
                                                <form action="{{ route('pengurus.surat.kirim', $surat->id) }}" method="POST" onsubmit="return confirm('Kirim surat ini sekarang? Setelah dikirim, tidak bisa dibatalkan.');">
                                                    @csrf
                                                ...
                                                <form action="{{ route('pengurus.surat.destroy', $surat->id) }}" method="POST" onsubmit="return confirm('Hapus draft ini?');">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-4 text-center text-gray-500">
                                            Belum ada surat yang Anda buat.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
